...
#class Output_get_en_Driver(BusDriver):
#    _signals = [
#            'EN_mv_get_decompressed_output',
#            ]
#
#    def __init__(self, dut):
#        BusDriver.__init__(self, dut, None, dut.CLK)
#
#class Output_get_en_Transaction(object):
#
#    def __init__(self,tb,
#            EN_mv_get_decompressed_output=0,
#            ):
#
#        self.EN_mv_get_decompressed_output = BinaryValue(EN_mv_get_decompressed_output,
#                tb.EN_width, False)
#
...
#    while True:
#        #yield RisingEdge(dut.CLK)
#        if(dut.RDY_mv_get_decompressed_output != 1):
#            yield RisingEdge(dut.RDY_mv_get_decompressed_output)
#        yield tb.outen_get_drv.send(Output_get_en_Transaction(tb,1))
#        yield FallingEdge(dut.RDY_mv_get_decompressed_output)
#        yield tb.outen_get_drv.send(Output_get_en_Transaction(tb,0))
#        #yield FallingEdge(dut.CLK)
#        #else:
#        #   yield RisingEdge(dut.CLK)
...
def run_test(dut):
    ...
    tb = Testbench(dut)
    cocotb.fork(en_decompressed_output(dut,tb))
